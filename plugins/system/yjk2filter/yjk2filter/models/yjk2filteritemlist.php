...
class K2Modelyjk2filteritemlist extends K2Model 
{
    ...
    { 
        ...
        switch ($task)
        {
			...
			//yjk2filter functionality - start
			case 'yjk2filter':

					//helpers to set the search values to session							
					$app 				= &JFactory::getApplication();
					$option 			= JRequest::getCmd('option');	
					
					$yjk2filter_search 	= JRequest::getVar('YJK2Filter', $app->getUserStateFromRequest($option.".".$task."[]", 'YJK2Filter', array(), 'array'), 'post', 'array');
					$app->setUserState($option.".".$task."[]", $yjk2filter_search);

					//get the group values in case the search is related to groups
					$yjk2filter_groups = JRequest::getInt('yjk2filter_groups', $app->getUserStateFromRequest($option.".".$task.".yjk2filter_groups", 'yjk2filter_groups', 0, 'int'));
					//set the values to session for the pagination 
					$app->setUserState($option.".".$task.".yjk2filter_groups", $yjk2filter_groups);

					//generate array with the ranged fields id
					$yjk2filter_range = JRequest::getString('yjk2filter_range', $app->getUserStateFromRequest($option.".".$task.".yjk2filter_range", 'yjk2filter_range', '', 'string'));
					//set the values to session for the pagination 
					$app->setUserState($option.".".$task.".yjk2filter_range", $yjk2filter_range);							

					$yjk2filter_ranged_fields 	= explode(",",$yjk2filter_range);
					$yjk2filter_search_value_empty = true;

					if(!empty($yjk2filter_search)){

						foreach($yjk2filter_search as $yjk2filter_search_row => $yjk2filter_search_value){
							//set the values to session for the pagination 

							if($yjk2filter_search_value != ""){
								$yjk2filter_search_value_empty = false;
								$field_id = explode("ExtraField_",$yjk2filter_search_row);
								settype($field_id[1],'integer');									
								
								//first check if the field is range
								if(in_array($field_id[1],$yjk2filter_ranged_fields) && $yjk2filter_search_value != ""){
									
									//get range start and end values
									$range_values = explode("-",$yjk2filter_search_value);											

									settype($range_values[0],'integer');
									settype($range_values[1],'integer');											

									//get the extra fields id for the ranged field
									$query_range = "SELECT i.id, i.extra_fields  FROM #__k2_items as i
											  WHERE i.extra_fields REGEXP BINARY '(.*{\"id\":\"".($field_id[1])."\",\"value\":\"(.*)\"}.*)'";
									$db->setQuery($query_range);
									$range_values_db = $db->loadObjectList();				
									...
									$match_range_id = array();
									if(!empty($range_values_db)){
										$json 			= new Services_JSON;
										foreach($range_values_db as $range_value_db){
											$price_all = $json->decode($range_value_db->extra_fields);

											foreach($price_all as $price) {
												if ( intval($field_id[1]) == intval($price->id) ) {
													//intval($price->value); exit;
													preg_match("/([0-9]+)(.+)([0-9]{2})/", $price->value, $matches);
													
													settype($matches[0],'float');
													settype($range_values[0],'float');
													settype($range_values[1],'float');

													if( $range_values[0] <= $matches[0] && $matches[0] <= $range_values[1]){
														$match_range_id[] = $range_value_db->id;
													}
													break;
												}
											}
									...
									//generate the sql for range value
									if(!empty($match_range_id)){
										$query .= " AND i.id IN (".implode(",",$match_range_id).")";
									}else{
										$query .= " AND i.id IN (NULL)";
									}
									
								//if $yjk2filter_search_value is array generate the new $yjk2filter_search_value as ["1","2","3","4"] with array values
								}elseif(is_array($yjk2filter_search_value) && !empty($yjk2filter_search_value)){
									$query_array_value = array();
									foreach($yjk2filter_search_value as $array_value){
										//["1","2","3","4"] - \"[^\\\"]*".$yjk2filter_search_value."[^\\\"]*\"
										$query_array_value[] = "\"[^\\\"]*".$array_value."[^\\\"]*\"";
									}
									$query .= " AND extra_fields REGEXP BINARY '(.*{\"id\":\"".($field_id[1])."\",\"value\":\[".implode(",",$query_array_value)."\]}.*)'";

								//if $yjk2filter_search_value is string and not empty
								}elseif($yjk2filter_search_value != ""){
									
									$query .= " AND extra_fields REGEXP BINARY '(.*{\"id\":\"".($field_id[1])."\",\"value\":\"".($yjk2filter_search_value)."\"}.*)'";
									//$query .= " AND extra_fields LIKE '%".$yjk2filter_search_value."%'";
									
								}
							}

							//add the group filter in case the search is based on groups
							if($yjk2filter_groups > 0){
								$query .= " AND catid IN (SELECT id FROM #__k2_categories WHERE extraFieldsGroup = ".$yjk2filter_groups.")";
							}										
						}//end foreach
					}

					if($yjk2filter_search_value_empty === true){
						//$query .= " AND i.id IN (NULL)";								
					}

					break;
			//yjk2filter functionality - end
